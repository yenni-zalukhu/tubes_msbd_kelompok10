...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
      ...
      <!-- Menampilkan jumlah produk aktif dengan custom warna -->
      <div class="badge" style="background-color: #B0A695; color: white;">
        Active Products : <strong>{{ $activeProductCount }}</strong>
      </div>
        ...
        <table class="table table-bordered" id="product-dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            @foreach($products as $product)
              @php
                $sub_cat_info = DB::table('categories')->select('title')->where('id', $product->cat_id)->first();
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form method="POST" action="{{ route('product.destroy', [$product->id]) }}">
                          @csrf
                          @method('delete')
                          <button class="btn btn-danger btn-sm dltBtn" data-id="{{ $product->id }}" style="height:30px; width:30px; border-radius:50%" data-toggle="tooltip" data-placement="bottom" title="Delete">
